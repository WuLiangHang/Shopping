...
//查询所有的管理员信息
  ...
  public ActionForward managerCheck(ActionMapping mapping,
                                    ...
                                    HttpServletRequest request,
                                    HttpServletResponse response) {
    String account =request.getParameter("account");		//获得输入的帐号
    ManagerForm managerForm= dao.selectOne(account);		//根据帐号查询管理员信息
    if (managerForm == null) {		//判断查询是否存在
      request.setAttribute("result", "您输入的账号不存在！！！");
      return mapping.findForward("checkResult");		
    }
    else if (!managerForm.getPassword().equals(request.getParameter("password"))) {//判断密码是否正确
