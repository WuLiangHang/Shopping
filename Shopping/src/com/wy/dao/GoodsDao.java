...
//添加操作
  public void insertGoods(GoodsForm form) {
    try {
      ...
      ps.setInt(10, 0);
      ps.setString(11, form.getMember());
      ps.executeUpdate();
      ps.close();
    }
    catch (SQLException ex) {
...
//更新操作
  public void updateGoods(GoodsForm form) {
    try {
      ps = connection.prepareStatement("update tb_goods set bigId=?,smallId=?,goodsName=?,goodsFrom=?,introduce=?,cretetime=?,nowPrice=?freePrice=?,number=?,pirture=?,mark=?,member=? where id=?");
      ps.setInt(1, form.getBig().intValue());
      ps.setInt(2, form.getSmall().intValue());
      ps.setString(3, form.getName());
      ps.setString(4, form.getFrom());
...
//根据商品名称搜索商品
  public List selectGoods(String goodname) {
    List list = new ArrayList();
    GoodsForm goods = null;
    try {
      ps = connection.prepareStatement("select * from tb_goods where goodsname like '%"+goodname+"%' order by id DESC");
...
//根据商家搜索商品
  public List selectGoodsByMember(String member) {
    List list = new ArrayList();
    GoodsForm goods = null;
    try {
      ps = connection.prepareStatement("select * from tb_goods where member = '"+member+"' order by id DESC");
      ...
      while (rs.next()) {
        ...
        goods.setPriture(rs.getString(11));
        goods.setMark(Integer.valueOf(rs.getString(12)));
        goods.setMember(rs.getString(13));
        list.add(goods);
      }
    }
    catch (SQLException ex) {
