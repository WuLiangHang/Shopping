...
public class OrderAction
  ...
  public ActionForward execute(ActionMapping mapping,
    ...
    case 4: {
        return selectOrderByMember(mapping, form, request, response); //查询卖家所有的订单
      }
    case 5: {
        return selectOrderSendByMember(mapping, form, request, response); //查询所有的订单
      }
    case 6: {
    	return selectOrderByName(mapping, form, request, response); //查询买家所有的订单
    }
...
//出货
  public ActionForward selectOrderSendByMember(ActionMapping mapping,
                                       ActionForm form,
                                       HttpServletRequest request,
                                       HttpServletResponse response) {
    order.updateSignOrder(request.getParameter("number"));
    return selectOrderByMember(mapping, form, request, response);
  }
...
//查询用户购买的物品订单
  public ActionForward selectOrderByName(ActionMapping mapping,
                                   ActionForm form,
                                   HttpServletRequest request,
                                   HttpServletResponse response) {
    Integer sign = null;
    ...
    List list = order.selectOrderSignByName(sign,name);
    int pageNumber = list.size(); //计算出有多少条记录
    int maxPage = pageNumber; //计算有多少页数
    String number = request.getParameter("i");
    if (maxPage % 6 == 0) {
...
//查询用户卖出的物品订单
  public ActionForward selectOrderByMember(ActionMapping mapping,
                                   ActionForm form,
                                   HttpServletRequest request,
                                   HttpServletResponse response) {
    Integer sign = null;
    ...
    List list = order.selectOrderSignByMember(sign,member);
    int pageNumber = list.size(); //计算出有多少条记录
    int maxPage = pageNumber; //计算有多少页数
    String number = request.getParameter("i");
    if (maxPage % 6 == 0) {
