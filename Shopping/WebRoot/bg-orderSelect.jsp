...
<br>
	...
	<table width="610" height="25" border="0" cellpadding="0" cellspacing="0" background="image/bg_02.jpg">
      <tr>
        ...
        <%if(request.getAttribute("type")==null&&session.getAttribute("isadmin").equals("true")){%>
        订单的查询
        <%}else if(request.getAttribute("type").equals("buy")){%>
        已买到的宝贝
        <%}else{%>
        已卖出的宝贝
        <%} %>
	    ...
	    <tr bgcolor="#DCDCDC">
          ...
          <td width="14%"><div align="center">买家</div></td>
          <td width="14%"><div align="center">卖家</div></td>
      ...
      OrderForm form=(OrderForm)list.get(i);
        ...
        <tr align="center" bgcolor="#FFFFFF">
          ...
          <%if(session.getAttribute("isadmin").equals("false")) {%>
	  			<%if(form.getSign().equals("0")){%>
                    <a href="orderAction.do?action=4&sign=<%=form.getSign()%>&member=<%=((MemberForm)session.getAttribute
...
("form")).getName() %>">是</a>
        ...
        <td><a href="orderAction.do?action=3&number=<%=form.getNumber()%>">详细信息</a>
         ...
         if(request.getAttribute("type").equals("sell")){%>
         <%if(form.getSign().equals("1")){ %>
         已出
         <%}else{ %>
         <a href="orderAction.do?action=5&number=<%=form.getNumber()%>&member=<%=((MemberForm)session.getAttribute("form")).getName() %>">出货</a>
         <%}
            }
         }%>
		 &nbsp;&nbsp;
