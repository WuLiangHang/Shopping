...
public class GoodsAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		case 17:{
			return goodSelectGoodsName(mapping, form, request, response);
		}
		case 18:{
			return goodSelectByMember(mapping, form, request, response);
		}
		case 19: {
			return goodUpdate(mapping, form, request, response); 
		}
		}
	...
	public ActionForward saveGoods(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		UploadFile uploadFile = new UploadFile();		//创建上传图片功能类
		GoodsForm goodsForm = (GoodsForm) form;			//声明商品表单

			
		String dir = servlet.getServletContext().getRealPath("/goodsPicture");		//获得上传图片的根目录
		FormFile formFile = goodsForm.getFormFile();		//获得表单信息
		String getType = formFile.getFileName().substring(		//获得类型
				formFile.getFileName().lastIndexOf(".") + 1);		
		String result = "添加商品信息失败";
		String imageType[] = { "JPG", "jpg", "gif", "bmp", "BMP","png","PNG" };		//设置上传图片的格式。
		for (int ii = 0; ii < imageType.length; ii++) {
			if (imageType[ii].equals(getType)) {
				
			    goodsForm.setBig(Integer.valueOf(request.getParameter("big")));		//设置大类别
			    goodsForm.setSmall(Integer.valueOf(request.getParameter("small")));		//设置小类别
			    goodsForm.setName(request.getParameter("name"));		//设置名称
			    goodsForm.setFrom(request.getParameter("from"));			//设置厂家
			    goodsForm.setNowPrice(Float.valueOf(request.getParameter("nowPirce")));		//设置价格
			    goodsForm.setFreePrice(Float.valueOf(request.getParameter("freePirce")));		//设置特价
			    goodsForm.setIntroduce(request.getParameter("introduce"));					//设置商品介绍
				
				goodsForm.setPriture("goodsPicture/"+uploadFile.upload(dir, formFile));
				goodsForm.setMember(request.getParameter("member"));
	...
	// 特价商品
	public ActionForward goodSelectGoodsName(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) {
		List list = null;
		String goodsName = request.getParameter("goodsName");
		list = dao.selectGoods(goodsName);
		int pageNumber = list.size(); // 计算出有多少条记录
		int maxPage = pageNumber; // 计算有多少页数
		String number = request.getParameter("i");
		if (maxPage % 4 == 0) {
		...
		request.setAttribute("pageNumber", String.valueOf(pageNumber));
		request.setAttribute("list", list);
		return mapping.findForward("goodSelectGoodsName");
	}
	
	// 
	public ActionForward goodSelectByMember(ActionMapping mapping,
			ActionForm form, HttpServletRequest request,
			HttpServletResponse response) {
		List list = null;
		String member = request.getParameter("member");
		list = dao.selectGoodsByMember(member);
		int pageNumber = list.size(); // 计算出有多少条记录
		int maxPage = pageNumber; // 计算有多少页数
		String number = request.getParameter("i");
		if (maxPage % 4 == 0) {
